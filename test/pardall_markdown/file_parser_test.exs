defmodule PardallMarkdown.FileParserTest do
  ...
  test "file without metadata is correctly parsed", %{current_config: current_config} do
    ...
    {:error, "could not find separator --- in \"./test/individual_content/no_metadata.md\""} =
      FileParser.extract!("./test/individual_content/no_metadata.md")
  end

  test "publishing status correctly set depending on the configuration", %{
    current_config: current_config
  } do
    Application.put_env(
      :pardall_markdown,
    ...
    {:ok, %Post{is_published: true}} =
      FileParser.extract!("./test/individual_content/set_published.md")

    {:ok, %Post{is_published: false}} =
      FileParser.extract!("./test/individual_content/dont_set_published.md")

    {:ok, %Post{is_published: false}} =
      FileParser.extract!("./test/individual_content/set_unpublished.md")
  ...
  @tag :joplin
  test "Joplin MetadataParser", %{current_config: current_config} do
    Application.put_env(
      :pardall_markdown,
      PardallMarkdown.Content,
      Keyword.merge(current_config,
        metadata_parser: PardallMarkdown.MetadataParser.JoplinNote,
        is_markdown_metadata_required: false,
        is_content_draft_by_default: false
      )
    )

    {:ok, %Post{title: "This is the post title"}} =
      FileParser.extract!("./test/individual_content/joplin/post_title.md")
    ...
    {:ok,
     %Post{
       ...
       content: "<p>\nContent should be here?</p>"
     }} = FileParser.extract!("./test/individual_content/joplin/no_title_at_top.md")
  end
